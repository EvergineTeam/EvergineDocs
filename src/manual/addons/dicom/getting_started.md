...
We provide the `Dicom.CustomRenderPath` that you can setup in two different ways:
...
In order to load a Dicom file call the `DicomComponent.LoadFromFile` member. The `DicomComponent` acts as a holder for a Dicom file.
...
The `DicomComponent` has the `DitheringEnabled` to enable this feature.

## 3D Visualization

In order to visualize Dicoms in 3D you need:
